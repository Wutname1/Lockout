...
function CreateInterruptMark(interruptID)
	...
	local spellIconPath = C_Spell.GetSpellTexture(interruptID)
	if debugStatements then
	...
	-- Get the spell name
	local spellData = C_Spell.GetSpellInfo(interruptID)
	interruptMark.spellName = spellData.name
	...
	ticker =
		...
		function()
			...
			if debugStatements then
				print('Remaining cooldown for', spellData.name, ':', remaining)
			end
			if remaining <= 0 then
	...
	function(self, event, ...)
		...
		elseif event == 'COMBAT_LOG_EVENT_UNFILTERED' then
			local timestamp,
				...
				extraSpellId,
				extraSpellName,
				extraSchool = CombatLogGetCurrentEventInfo()
			...
			if interruptedUnit and interruptedStartTime and interruptedEndTime then
				...
				if interruptID then
					if not interruptMark then
						...
						if #Lockout.interruptMarks < 3 then
